...
def messages_view(request):
    ...
    user_messages = Message.objects.filter(receiver=actual_user)
    return render(request, 'messages.html', {'messages': user_messages})

# @login_required(login_url='login')
def courses_list_view(request):
    courses = Course.objects.all()
    enrollments = Enrollment.objects.filter(student=request.user.id)
    ...
    if request.method == 'POST':
        for enrollment in enrollments:
            ...
            if grade is not None:
                try:
                    # Try converting the input to a valid numeric value
                    numeric_grade = float(grade)
                    enrollment.grade = numeric_grade
                    enrollment.save()
                except ValueError:
                    # Handle the case where the input is not a valid numeric value
                    messages.error(request, f'Invalid grade value for {enrollment.student.username}.')
                    # You might want to redirect to the same page or handle this error appropriately
